...
static NSString *key = @"aetuiopljn";
static char *iv = "12345678";
...
+ (NSData *)dataWithBase64EncodedString:(NSString *)string {
  ...
  while (YES) {
    ...
    if (bufferLength ==
        1)  //  At least two characters are needed to produce one byte!
    {
      free(bytes);
      return nil;
    }
...
+ (NSString *)base64EncodedStringFrom:(NSData *)data {
  ...
  while (i < [data length]) {
    ...
    characters[length++] =
        encodingTable[((buffer[0] & 0x03) << 4) | ((buffer[1] & 0xF0) >> 4)];
    if (bufferLength > 1)
      characters[length++] =
          encodingTable[((buffer[1] & 0x0F) << 2) | ((buffer[2] & 0xC0) >> 6)];
    else
      characters[length++] = '=';
    if (bufferLength > 2)
...
+ (NSData *)DESEncrypt:(NSData *)data WithKey:(NSString *)key {
  ...
  CCCryptorStatus cryptStatus = CCCrypt(
      kCCEncrypt, kCCAlgorithmDES, kCCOptionPKCS7Padding | kCCOptionECBMode,
...
+ (NSString *)hexStringFromString:(NSString *)string {
  ...
  for (NSString *toRemove in [NSArray arrayWithObjects:@"<", @">", @" ", nil])
    hexStr =
        [hexStr stringByReplacingOccurrencesOfString:toRemove withString:@""];
  return hexStr;
}
@end
