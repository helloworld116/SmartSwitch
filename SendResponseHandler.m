...
- (void)checkWithTag:(long)tag {
  if (kSharedAppliction.networkStatus != NotReachable) {
    switch (tag) {
      ...
      default: {
        NSString* key = [[XMLUtil sharedInstance] getMacByTag:tag];
        if ([[SendResponseHandler shareInstance].responseDictE
                objectForKey:key] == [NSNull null]) {
          if ([[[MessageUtil shareInstance].msgDSendCountDict
                  objectForKey:key] intValue] < kTryCount &&
              [[UdpSocketUtil shareInstance].delegate
                  respondsToSelector:@selector(noResponseMsgIdE:)]) {
            NSLog(@"tag %ld 重新发送%d次", tag,
                  [[[MessageUtil shareInstance].msgDSendCountDict
                      objectForKey:key] intValue] +
                      1);
            [[UdpSocketUtil shareInstance].delegate noResponseMsgIdE:key];
          }
        }
        break;
      }
