...
- (void)udpSocket:(GCDAsyncUdpSocket*)sock didSendDataWithTag:(long)tag {
  ...
  switch (tag) {
    ...
    default:
      [[SendResponseHandler shareInstance].responseDictE
          setObject:[NSNull null]
             forKey:[[XMLUtil sharedInstance] getMacByTag:tag]];
      break;
  }
  ...
  switch (tag) {
    ...
    default:
      if ([self.delegate respondsToSelector:@selector(noSendMsgIdBOrD:)]) {
        [self.delegate
            noSendMsgIdBOrD:[[XMLUtil sharedInstance] getMacByTag:tag]];
      }
      break;
  }
}
  ...
  if (data) {
    ...
    switch (msg.msgId) {
      ...
      case 0xe:
        ...
        //处理批量向外网发送的情况
        [[SendResponseHandler shareInstance].responseDictE setObject:data
                                                              forKey:msg.mac];
